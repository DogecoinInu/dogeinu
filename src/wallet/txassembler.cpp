    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional DOGEINU outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes DOGEINU and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::DOGEINU_TO_DOGEINU) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a DOGEINU-to-DOGEINU transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::DOGEINU_ONLY;
        ...
        // If DOGEINU-to-DOGEINU fails, create a peg-out transaction
