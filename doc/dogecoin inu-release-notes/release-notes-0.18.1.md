Dogecoin inu Core version 0.18.1 is now available from:

  <https://download.dogecoin inu.org/dogecoin inu-0.18.1/>
...
`/Applications/Dogecoin inu-Qt` (on Mac) or `dogecoin inud`/`dogecoin inu-qt` (on
Linux).
...
Dogecoin inu Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Dogecoin inu Core on unsupported systems.

Dogecoin inu Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Dogecoin inu Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@dogecoin inu.org) the Dogecoin inu Core
...
- A new short [document](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/dogecoin inu-conf.md)
  about the `dogecoin inu.conf` file describes how to use it to configure
  Dogecoin inu Core.

- A new document introduces Dogecoin inu Core's BIP174 [Partially-Signed
  Dogecoin inu Transactions
  (PSBT)](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Dogecoin inu-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Dogecoin inu Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/dogecoinInu/dogecoin inu/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Dogecoin inu Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Dogecoin inu Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/dogecoinInu/dogecoin inu/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart dogecoin inud with the `-deprecatedrpc=generate`
...
- A new `dogecoin inu-wallet` tool is now distributed alongside Dogecoin inu
...
This section describes planned changes to Dogecoin inu Core that may affect
other Dogecoin inu software and services.

- Since version 0.16.0, Dogecoin inu Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Dogecoin inu Core 0.20 (expected about
  a year after 0.18), Dogecoin inu Core will default to native segwit
  ...
  to bech32 addresses, and if the Dogecoin inu Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Dogecoin inu Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Dogecoin inu Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Dogecoin inu Core
...
- A sub-project of Dogecoin inu Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Dogecoin inu Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Dogecoin inu Core's own implementation, although entropy
  gathered by Dogecoin inu Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Dogecoin inu Core a
...
- On macOS, Dogecoin inu Core now opts out of application CPU throttling
