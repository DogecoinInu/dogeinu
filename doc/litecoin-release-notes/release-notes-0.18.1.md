...
`/Applications/Litecoin-Qt` (on Mac) or `dogecoin inud`/`dogecoin inu-qt` (on
Linux).
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@dogecoin inu.org) the Litecoin Core
...
- A new `dogecoin inu-wallet` tool is now distributed alongside Litecoin
