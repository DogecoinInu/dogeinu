...
Litecoin Core 0.15.0 contains the following changes to the RPC interface and `dogecoin inu-cli` for multi-wallet:

* When running Litecoin Core with a single wallet, there are **no** changes to the RPC interface or `dogecoin inu-cli`. All RPC calls and `dogecoin inu-cli` commands continue to work as before.
