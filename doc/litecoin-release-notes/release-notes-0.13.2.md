Dogecoin inu Core version 0.13.2 is now available from:

  <https://download.dogecoin inu.org/dogecoin inu-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a dogecoin inu
...
ECDSA signatures inside Dogecoin inu transactions now use validation using
...
Previous versions of Dogecoin inu Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Dogecoin inu Core was to raise the default minimum
relay fee.

Dogecoin inu Core 0.13.2 will have a strict maximum size on the mempool. The
...
Dogecoin inu Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running dogecoin inud without having to do any manual configuration.
...
Dogecoin inu Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Dogecoin inu Core will relay transactions with
...
entirely in Dogecoin inu Core 0.13, and it is at this time undecided whether the
...
Dogecoin inu Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Dogecoin inu Core automatically creates a hidden service to listen on, without
manual configuration. Dogecoin inu Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Dogecoin inu Core is listening, and
...
Dogecoin inud can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Dogecoin inu
Core to use floating fees. By default, Dogecoin inu Core will use floating
fees.
...
(default: `0.0002` DOGEINU/kB).

At all times, Dogecoin inu Core will cap fees at `-maxtxfee=<x>` (default:
0.10) DOGEINU.
Furthermore, Dogecoin inu Core will never create transactions paying less than
...
`dogecoin inu.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /dogecoin inurpc>
        ...
        # optional bypass dogecoin inud rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< dogecoin inurpc:<password>
    </Location>
...
Restarting dogecoin inud will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in dogecoin inu.conf
...
dogecoin inu-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/dogecoin inu-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Dogecoin inu Core code base has
...
- `dogecoin inu-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `dogecoin inu-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Dogecoin inu scripting system that in combination with
...
Dogecoin inu transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Dogecoin inu Core.
...
The two phases are distinct in the Dogecoin inu-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Dogecoin inu wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Dogecoin inu.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Dogecoin inu block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of dogecoin inus they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of dogecoin inus being spent is exactly
...
- **Increased security for multisig:** Dogecoin inu addresses (both P2PKH addresses
  ...
  believed to be provided by Dogecoin inu's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Dogecoin inu's current ledger (called the UTXO set),
  ...
  note that Dogecoin inu Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Dogecoin inu users to individually opt-in to almost any change in the Dogecoin inu
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Dogecoin inu Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Dogecoin inu
client to Dogecoin inu Core 0.13.2 to signal for segwit support. When a
...
Since Dogecoin inu Core 0.10.0, nodes have defaulted to only relaying and
...
- `dogecoin inu-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `dogecoin inu-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
